 ...
 */
window.ForceEdgeBundling = function () {
    ...
    function custom_edge_length(e) {
        return Math.sqrt(Math.pow(e.source.x - e.target.x, 2) + Math.pow(e.source.y - e.target.y, 2) + Math.pow(e.source.z - e.target.z, 2));
    }
    ...
    function project_point_on_line(p, Q) {
        var L = Math.sqrt((Q.target.x - Q.source.x) * (Q.target.x - Q.source.x) + (Q.target.y - Q.source.y) * (Q.target.y - Q.source.y) + (Q.target.y - Q.source.y) * (Q.target.z - Q.source.z));
        var r = ((Q.source.y - p.y) * (Q.source.y - Q.target.y) - (Q.source.x - p.x) * (Q.target.x - Q.source.x) - (Q.source.z - p.z) * (Q.target.z - Q.source.z)) / (L * L);

        return {
            'x': (Q.source.x + r * (Q.target.x - Q.source.x)),
            'y': (Q.source.y + r * (Q.target.y - Q.source.y)),
            'z': (Q.source.z + r * (Q.target.z - Q.source.z))
        };
    }
    ...
    function filter_self_loops(edgelist) {
        ...
        for (var e = 0; e < edgelist.length; e++) {
            if (data_nodes[edgelist[e].source].x != data_nodes[edgelist[e].target].x ||
                    ...
                    data_nodes[edgelist[e].source].y != data_nodes[edgelist[e].target].z) { //or smaller than eps
    ...
    function compatibility_score(P, Q) {
        ...
        return (angle_compatibility(P, Q) * scale_compatibility(P, Q) * position_compatibility(P, Q));
    }
