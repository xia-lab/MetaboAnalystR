...
CreateTandemMSAnalysisRnwReport_slides <- function(mSetObj, usrName){
  ...
  CreateMS2Intr_slides();
  CreateMS2SpecIOdoc_slides(mSetObj);
  CreateMS2AnalMethod_slides(mSetObj);
  CreateMS2AnalResults_slides(mSetObj);
  CreateMatchingPatterns_slides(mSetObj);
...
CreateMS2SpecIOdoc_slides <- function(mSetObj){
...
CreateMS2AnalMethod_slides <- function(mSetObj){
...
CreateMS2AnalResults_slides <- function(mSetObj){
  ...
  cmdhist2 <- c(
    ...
    "sum_dt <- createMS2SumTableRMD_slides(mSetObj);",
...
createMS2SumTableRMD_slides <- function(mSetObj){
  save(mSetObj, file = "mSetObj_createMS2SumTableRMD.rda")
  ...
  if(!is.null(mSetObj$dataSet$spectrum_dataframe)){
    if(length(mSetObj[["dataSet"]][["msms_result"]][[1]][["IDs"]])>0){
      ...
      res_df <- data.frame(Ranks = ranks, Scores = Scores,
    ...
    res_df_list <- lapply(1:length(mSetObj[["dataSet"]][["msms_result"]]), function(x){
      ...
      if(length(Indexs)==0){
        ...
        res_dx <- data.frame(Index = ID, Features = featurelabel, Ranks = ranks, Scores = Scores,
                             Similarity = dot_products, Compounds = compounds,
...
CreateMatchingPatterns_slides <- function(mSetObj){
