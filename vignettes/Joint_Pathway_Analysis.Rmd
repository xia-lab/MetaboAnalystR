...
Before beginning, **two files must be downloaded** and placed in your working directory, the [gene list]() and the [metabolite list](https://github.com/xia-lab/MetaboAnalystR/blob/master/data/integ_cmpds.txt). The example data comes from an integrative analysis of the transcriptome and metabolome to identify (metabolites/genes) biomarkers of intrahepatic cholangiocarcinoma (ICC) in 16 individuals. Please download these files prior to using the tutorial below.  
