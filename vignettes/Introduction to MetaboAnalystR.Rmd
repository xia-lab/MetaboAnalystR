---
title: "The MetaboAnalystR Package"
author: "Jasmine Chong, Jeff Xia"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{MetaboAnalystR}
...
We will first begin with an example dataset included in the package called "cachexia_concentrations.csv". To begin the analysis, we first have to create an object for data processing, using the function *InitDataObjects*, and then we will read in the data file using the function *Read.TextData*. 
...
Note that *dataSet$read.msg* can be used to view the messages saved from importing the dataset. 
...
mSet <- UnzipUploadedFile(mSet, "http://www.metaboanalyst.ca/resources/data/lcms_netcdf.zip", "upload", T)
...
Similarly to binned spectral data, there is a further step to preprocess MS spectra data (2.1.4 Preprocessing example data (LC/GC-MS spectra)). 
