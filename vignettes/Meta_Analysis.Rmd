...
setwd("~/Desktop/R_metaboanalyst_bitbucket/data")
...
mSet <- ReadIndData(mSet, "adenocarcinoma_data2.csv", "colu");

# Sanity check data to ensure it is ready for analysis
mSet <- SanityCheckIndData(mSet, "adenocarcinoma_data2.csv")
...
mSet <- PerformIndNormalization(mSet, "adenocarcinoma_data2.csv", "log", 1);

#Perform differential expression analysis to identify DE features
mSet <- PerformLimmaDE(mSet, "adenocarcinoma_data2.csv", 0.05, 0.0);

# Repeat steps for example data3
mSet <- ReadIndData(mSet, "adenocarcinoma_data3.csv", "colu");
mSet <- SanityCheckIndData(mSet, "adenocarcinoma_data3.csv")
mSet <- PerformIndNormalization(mSet, "adenocarcinoma_data3.csv", "log", 1);
mSet <- PerformLimmaDE(mSet, "adenocarcinoma_data3.csv", 0.05, 0.0);

# Repeat steps for example data4
mSet <- ReadIndData(mSet, "adenocarcinoma_data4.csv", "colu");
mSet <- SanityCheckIndData(mSet, "adenocarcinoma_data4.csv")
mSet <- PerformIndNormalization(mSet, "adenocarcinoma_data4.csv", "log", 1);
mSet <- PerformLimmaDE(mSet, "adenocarcinoma_data4.csv", 0.05, 0.0);
...
# prepare data for Venn Diagram, which will create a Integrated Venn diagram in your working directory (two overlapping circles, highlighting novel biomarker features from the meta-analysis, biomarkers that were consistently identified using meta-analysis and individual DE expression, and biomarkers that were only identified using individual DE expression.)
mSet <- PrepareVennData(mSet);
mSet <- GetSelectedDataNumber(mSet);
mSet <- GetSelectedDataNames(mSet);

# get names of features overlapping between selected datasets
mSet <- GetVennGeneNames(mSet, "adenocarcinoma_data2.csvadenocarcinoma_data3.csvmeta_dat");
mSet <- GetVennGeneNames(mSet, "adenocarcinoma_data2.csv||adenocarcinoma_data2.csvadenocarcinoma_data3.csvmeta_dat");
