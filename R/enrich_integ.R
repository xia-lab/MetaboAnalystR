...
PlotInmexPath <- function(mSetObj=NA, pathName, width=NA, height=NA, format="png", dpi=NULL){
  ...
  if(.on.public.web){ 
    ...
    if(.on.public.web){ PlotInmexGraph(NA, pathName, g, width, height, bg.cols, line.cols, format, dpi); mSetObj <- .get.mSet(mSetObj) } else { mSetObj <- PlotInmexGraph(mSetObj, pathName, g, width, height, bg.cols, line.cols, format, dpi) };
