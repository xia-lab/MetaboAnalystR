...
PrepareMetaPath <- function(mSetObj = NA) {
  ...
  mSetObj<-CalculateEffectSize(mSetObj, FALSE, "hodges")
  mSetObj<-FC.Anal.unpaired(mSetObj, 2.0, 0)
  m1 <- .get.mSet(mSetObj);
  mSetObj<-Convert2MummichogMetaPath(m1, rt=TRUE, rds.file=FALSE, "seconds", "all", "positive") 
...
MetaPathNormalization <- function(mSetObj = NA, sampleNor, tranform, scale){
  ...
  # TODO: enable the PCA later
  # mSetObj <- PCA.Anal(mSetObj)
  # mSetObj <- PlotPCAPairSummary(mSetObj, "pca_pair_0_", "png", 72, width=NA, 5)
  # mSetObj <- PlotPCAScree(mSetObj, "pca_scree_0_", "png", 72, width=NA, 5)
  # mSetObj <- PlotPCA2DScore(mSetObj, "pca_score2d_0_", "pdf", 72, width=NA, 1,2,0.95,1,0)
  # 
}
...
Convert2MummichogMetaPath <- function(mSetObj=NA, rt=FALSE, rds.file=FALSE, rt.type="seconds", 
  ...
  if(test=="tt"|test=="all"){
    ...
    message("Running here successfully!-------------2")
...
###################################################################
# Funtions for Meta-Analysis + Visualization for R package
