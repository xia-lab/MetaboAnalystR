...
#' @export
#'
SetSelectedMetaInfo <- function(mSetObj=NA, meta0, meta1, block1){
...
#' @license MIT
#'
PerformDoseDEAnal<-function(mSetObj=NA, anal.type = "default", par1 = NULL, par2 = NULL, nested.opt = "intonly", robustTrend=F){

  mSetObj <- .get.mSet(mSetObj);  
  dataSet <- mSetObj$dataSet;

  dataSet <- prepareContrast(dataSet, anal.type, par1, par2, nested.opt);
  dataSet <- .perform_limma(dataSet, robustTrend);
  mSetObj$dataSet <- dataSet;
  return(.set.mSet(mSetObj));
}
...
.perform_limma <- function(dataSet, robustTrend = F){
  design <- dataSet$design;
...
SetupDesignMatrix<-function(mSetObj=NA, deMethod){
  mSetObj <- .get.mSet(mSetObj);  
  dataSet <- mSetObj$dataSet;
...
#'@export
#'
PerformLimmaDE<-function(mSetObj=NA, grps, p.lvl, fc.lvl=NULL){
  mSetObj <- .get.mSet(mSetObj);  
  dataSet <- mSetObj$dataSet;
