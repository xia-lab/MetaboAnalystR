...
{
  # Heavy matrix computation — run in subprocess
  result <- rsclient_isolated_exec(
    func_body = function(input_data) {
      ...
      beta.d <- input_data$beta.d
      beta <- input_data$beta
      alpha.d <- input_data$alpha.d
      alpha <- input_data$alpha
      condition.grp <- input_data$condition.grp
      time.grp <- input_data$time.grp
      rep.grp <- input_data$rep.grp
      p <- input_data$p

      # Inject helper functions
      matrix.cov <- input_data$.matrix.cov
      mb.squeezeVar <- input_data$.mb.squeezeVar
      mb.fitFDist <- input_data$.mb.fitFDist
      mb.trigammaInverse <- input_data$.mb.trigammaInverse
      ot.helmert <- input_data$.ot.helmert
    ...
    input_data = list(
      object = object, times = times, D = D, size = size,
      nu = nu, Lambda = Lambda, beta.d = beta.d, beta = beta,
      alpha.d = alpha.d, alpha = alpha, condition.grp = condition.grp,
      time.grp = time.grp, rep.grp = rep.grp, p = p,
      .matrix.cov = matrix.cov,
      .mb.squeezeVar = mb.squeezeVar,
      .mb.fitFDist = mb.fitFDist,
      .mb.trigammaInverse = mb.trigammaInverse,
      .ot.helmert = ot.helmert
    ),
    packages = c("stats"),
    timeout = 120,
    output_type = "qs"
