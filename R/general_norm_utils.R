...
#'Two plot summary plot: Feature View of before and after normalization with AI-enhanced customization
#'@description For each plot, the top is a box plot, bottom is a density plot with AI-optimized parameters
#'@usage PlotNormSummaryAI(mSetObj, imgName, format, dpi, width, customParams)
#'@param mSetObj Input the name of the created mSetObj (see InitDataObjects)
#'@param imgName Input a name for the plot
#'@param format Select the image format, "png", or "pdf". 
#'@param dpi Input the dpi. If the image format is "pdf", users need not define the dpi. For "png" images, 
...
#'@param width Input the width, there are 2 default widths, the first, width = NULL, is 10.5.
#'The second default is width = 0, where the width is 7.2. Otherwise users can input their own width.
#'@param customParams Additional R parameters for customization (e.g., colors, fonts, margins)
#'@author Jeff Xia \email{jeff.xia@mcgill.ca}, Jasmine Chong 
#'McGill University, Canada
#'@export
#'
PlotNormSummaryAI <- function(mSetObj=NA, imgName, format="png", dpi=72, width=NA, customParams=""){
  ...
  if(customParams != "") {
    tryCatch({
      eval(parse(text=customParams))
    }, error = function(e) {
      warning("Error parsing custom parameters, using defaults")
    })
  }
  
  # Default parameters that can be overridden by customParams
  density_col <- "darkblue"
  boxplot_col <- "lightgreen"
  density_lwd <- 2
  boxplot_lwd <- 1
  label_cex <- 1
  title_cex <- 1.2
  margin_mar <- c(4,7,4,0)
  ...
  # fig 1
  if(anal.type == "roc" & mSetObj$dataSet$roc_cols == 1){
    op<-par(mar=margin_mar, xaxt="s");
    plot.new()
  }else{
    op<-par(mar=margin_mar, xaxt="s");
    plot(density(apply(proc.data, 2, mean, na.rm=TRUE)), 
         col=density_col, las=2, lwd=density_lwd, 
         main="", xlab="", ylab="", cex.lab=label_cex);
    mtext("Density", 2, 5, cex=label_cex);
    mtext("Before Normalization", 3, 1, cex=title_cex)
  }
  
  # fig 2
  op<-par(mar=margin_mar, xaxt="s");
  boxplot(proc.data[,pre.inx, drop=FALSE], 
          names=namesVec, ylim=rangex.pre, las=2, 
          col=boxplot_col, horizontal=T, show.names=T,
          cex.axis=label_cex, lwd=boxplot_lwd);
  mtext(x.label, 1, 5, cex=label_cex);
  ...
  if(anal.type == "roc" & mSetObj$dataSet$roc_cols == 1){
    ...
    op<-par(mar=margin_mar, xaxt="s");
    plot(density(apply(norm_dt, 2, mean, na.rm=TRUE)), 
         col=density_col, las=2, lwd=density_lwd, 
         main="", xlab="", ylab="", cex.lab=label_cex);
    mtext("After Normalization", 3, 1, cex=title_cex);
  }

  # fig 4
  op<-par(mar=margin_mar, xaxt="s");
  boxplot(norm_dt[,norm.inx, drop=FALSE], 
          names=namesVec, ylim=rangex.norm, las=2, 
          col=boxplot_col, horizontal=T, show.names=T,
          cex.axis=label_cex, lwd=boxplot_lwd);
  mtext(paste("Normalized",x.label), 1, 5, cex=label_cex);
