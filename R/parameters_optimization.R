...
PerformParamsOptimization<-function(raw_data, param=p0, method="DoE", ncore=4){
  ...
  # Define the global Parallel cores for BiocParallel function
  ...
  if (param[["Peak_method"]] == "centWave"){
    ...
    param[["ppm"]]<-p2$ppm
    param[["noise"]]<-p2$noise;
    param[["prefilter"]]<-p2$prefilter;
    param[["value_of_prefilter"]]<-p2$value_of_prefilter;
